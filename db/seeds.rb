...
hyper_coasterbm = Product.find_or_create_by!(
	...
	can_invert: false,
	launch_options: ["flat launch (see Thunderbird - Holiday World)", "inclined launch (see Incredible Hulk)"],
	typical_elements: ["small fast floater hill (see Shambala/Flying Wing Coaster - Happy Valley)", "trim brakes"],
	plausible_elements: ["watersplash", "ampersand (see Shambala)", "long V-trains (see Diamondback)", "intense helix (see Nitro)", "double-down", "double-up", "helix turnaround (see Goliath SFOG)", "B&M dip (see Raging Bull/Apollo's Chariot", "overbanked turn (see Silver Bullet)", "S-turnaround (see Intimidator)", "Fury 325 turnaround that ends at 90 degrees", "figure-8 finale (see Raging Bull)", "flat turnaround (see Goliath - La Ronde)", "high in the air turn (see Superman SFFT, Thunderbird - Holiday World, Rougarou)", "dipping first drop (see Green Lantern/Rougarou)", "S-hill (see Rougarou)", "trenches", "stengal dive"]
)
...
giga_coasterbm = Product.find_or_create_by!(
	...
	plausible_elements: ["watersplash", "ampersand (see Shambala)", "intense helix (see Nitro)", "double-down", "double-up", "helix turnaround (see Goliath SFOG)", "B&M dip (see Raging Bull/Apollo's Chariot", "overbanked turn (see Silver Bullet)", "S-turnaround (see Intimidator)", "Fury 325 turnaround that ends at 90 degrees", "figure-8 finale (see Raging Bull)", "flat turnaround (see Goliath - La Ronde)", "high in the air turn (see Superman SFFT, Thunderbird - Holiday World, Rougarou)", "S-hill (see Rougarou)", "trenches", "ampersand (see Shambala)", "first drop ends in a turn"],
	can_invert: false
)
...
arrow_looper = Product.find_or_create_by!(
	...
	old: true,
	can_launch: false,
	typical_elements: ["double loop", "double corkscrew", "batwing"],
...
vekoma_slc =  Product.find_or_create_by!(
	...
	difficult: false,
	can_launch: false,
	typical_elements: ["roll over (sea serpent on the SLC model)", "immelman"],
...
intamin_invert = Product.find_or_create_by!(
	...
	plausible_elements: ["overbanked turn", "immelman", "dive loop", "flying snake dive", "sea serpent", "tunnels", "trenches", "multiple-launches", "cobra roll", "corkscrew", "helix", "stengal dive", "reverse flying snake dive"]
)
...
intamin_looper = Product.find_or_create_by!(
	...
	plausible_elements: ["overbanked turn", "immelman", "dive loop", "flying snake dive", "sea serpent", "tunnels", "trenches", "multiple-launches", "norwegian loop (see Speed Monster)", "zero-g-roll", "stengal dive", "helix", "air hill", "s-hill", "reverse flying snake dive"]
)
...
intamin_lsm = Product.find_or_create_by!(
	...
	difficult: true,
	must_launch: true,
	launch_options: ["flat launch", "turbo lifthill over-a-hill (see Maverick)", "triple-launch w/spike (see Soaring With Dragon)"],
	typical_elements: ["tophat", "overbanked turn", "s-hills"],
	plausible_elements: ["vertical lift w/beyond vertical drop (see Fahrenheit)", "immelman", "dive loop", "flying snake dive", "sea serpent", "tunnels", "trenches", "multiple-launches (make it like Cheetah Hunt)", "norwegian loop (see Speed Monster)", "s-hills", "air hill", "cobra roll", "corkscrew", "vertical loop", "beyond vertical drop (see Maverick)", "double-up", "double-down", "non-inverting loop (see Flying Aces)", "zero-g-roll", "ultra-compact (see Taron)", "reverse flying snake dive"]
)
...
intamin_accelerator = Product.find_or_create_by!(
	...
	typical_elements: ["tophat", "overbanked turn", "s-hills"],
	plausible_elements: ["immelman", "dive loop", "flying snake dive", "sea serpent", "tunnels", "trenches", "norwegian loop (see Speed Monster)", "inverted tophat", "cobra roll", "corkscrew", "vertical loop", "air hill", "double-up", "double-down", "non-inverting loop (see Flying Aces)", "zero-g-roll", "forget what else is written, no inversions!", "no tophat", "tophat doesn't turn train 180 degrees (see Storm Runner)", "reverse flying snake dive"]
)
...
gerst_bobsled = Product.find_or_create_by!(
	...
	plausible_elements: ["tunnels", "overbanked turn", "double-up", "double-down", 'outer-banked curve', "stengal dive", 'outer-banked curve']
)
...
gerst_spinner = Product.find_or_create_by!(
	...
	plausible_elements: ["tunnels", "overbanked turn", "double-up", "double-down", "corkscrew (see Veil of Dark - The Shooting Coaster)", "JUST KIDDING NOW MAKE IT A ZAMPERLA SPINNER AHAHAHAHAHA!!!!!", "inclined loop", "vertical turn", "stengal dive", "short boosts with tires (see Galaxy Orbiter)", 'outer-banked curve']
)
...
maurer_spinner = Product.find_or_create_by!(
	...
	plausible_elements: ["tunnels", "overbanked turn", "double-up", "double-down", 'outer-banked curve', "s-hill", "trenches", "stengal dive", "JUST KIDDING NOW MAKE IT A ZAMPERLA SPINNER AHAHAHAHAHA!!!!!", "inclined loop", "portable", "tilt-track (see Winjas)", "treble clef (see Hollywood Rip Ride Rockit", 'outer-banked curve']
)
...
premier_shuttle = Product.find_or_create_by!(
	...
	plausible_elements: ["cobra roll", "zero-g-roll", "dive loop", "immelman", "vertical loop", "corkscrew", "vertical spike", "stengal dive", "non-inverting loop", "s-hill", "air hill", "half-loop half-barrel roll", "non-inverting loop"]
)
...
premier_launcher = Product.find_or_create_by!(
	...
	plausible_elements: ["cobra roll", "zero g roll", "dive loop", "immelman", "vertical loop", "corkscrew", "barrel roll", "s-hill", "air hill", "loop is combined with another element like an air hill (see Full Throttle)", "stengal dive", "helix", "stall on the tophat (see Sky Rocket)", "new-gen tiny-trains (see Sky Rocket)", "spaghetti-bowl (see Joker's Jinx)",  "half-loop half-barrel roll", "non-inverting loop"]
)

mack_launcher = Product.find_or_create_by!(
	manufacturer_id: mack.id,
	product_name: "Mack Launch Coaster",
	tall: 131,
	short: 50,
	difficult: false,
	must_launch: true,
...
mack_mega_coaster = Product.find_or_create_by!(
	manufacturer_id: mack.id,
	product_name: "Mack Mega Coaster",
	tall: 202,
	short: 105,
	difficult: false,
	can_launch: false,
